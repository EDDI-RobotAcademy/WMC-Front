...
requestCreateReviewToSpring ({}, payload) {
  ...
  const { title, content, writer } = payload
  return axios.post('http://localhost:7777/review/register',
      { title, content, writer })
      .then((res) => {
          alert('리뷰 등록 성공: ' + JSON.stringify(res.data))
          return res
      })
      .catch(() => {
          alert('문제 발생!')
...
requestReviewToSpring ({ commit }, reviewId) {
  return axios.get(`http://localhost:7777/review/${reviewId}`)
      .then((res) => {
